...
Second, it illustrates the **danger of shared mutable state** in operating system networking stacks. Global rate limiters, global counters, and global queues that affect behavior across independent connections create side channels. These design patterns appear throughout network code, and their security implications are rarely analyzed at design time. The paper's methodology of looking for cross-connection observable effects of global kernel state is a template that can and should be applied to other protocols and subsystems.
